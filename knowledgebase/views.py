...
def dashboard(request):
    ...
    pub_cat_ids = [5, 14]
    count_publication = Document.objects.filter(Q(data_category__data_common_category_id=2)).count()    # Document.objects.filter(data_category__id__in=pub_cat_ids).count()
    ...
    count_law_act_policy = Document.objects.filter(Q(data_category__data_common_category_id=1)).count()    # Document.objects.filter(data_category__id__in=law_act_policy_rule_guidline_cat_ids).count()

    plan_cat_ids = [33, 34, 51]
    count_plan = Document.objects.filter(Q(data_category__data_common_category_id=4)).count()    # Document.objects.filter(data_category__id__in=plan_cat_ids).count()

    agreements_mous = [2, 12, 13, 63]
    count_agreements_mous = Document.objects.filter(Q(data_category__data_common_category_id=5)).count()    # Document.objects.filter(data_category__id__in=agreements_mous).count()         # Q(data_category__data_common_category_id=5)).count()  

    modeling_tools = [59, 60]
    count_modeling_tools = Document.objects.filter(Q(data_category__data_common_category_id=6)).count()    # data_category__id__in=modeling_tools).count()

    # workshops_seminar = [7]
    count_workshops_seminar = Document.objects.filter(Q(data_category__data_common_category_id=7)).count()                
    ...
    agree_mous_docs = Document.objects.filter(Q(data_category__data_common_category_id=5)).order_by('data_category_id')
    modeling_tools_docs = Document.objects.filter(Q(data_category__data_common_category_id=6)).order_by('data_category_id')
    workshops_seminar_docs = Document.objects.filter(Q(data_category__data_common_category_id=7)).order_by('data_category_id')   
    ...
    context = {'organization': count_organization,
               ...
               'iwm_docs': count_iwm, 'cegis_docs': count_cegis, 'cat_map': category_name_map, 'org_names': orgranization_name, 'acts_policy_rules_guidelines_docs' : acts_policy_rules_guidelines_docs, 'plans_docs' : plans_docs, 'research_pub_docs' : research_pub_docs, 'report_docs' : report_docs, 'agree_mous_docs': agree_mous_docs, 'modeling_tools_docs': modeling_tools_docs, 'workshops_seminar_docs': workshops_seminar_docs }
